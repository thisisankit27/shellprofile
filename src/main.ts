...
const LINKEDIN = command.social.linkedin.url;
const GITHUB = command.social.github.url;
const EMAIL = command.social.email.url;
...
function commandHandler(input : string) {
  ...
  switch(input) {
    ...
    case 'linkedin':
      ...
      setTimeout(() => {
        window.open('https://www.linkedin.com/in/' + encodeURIComponent(LINKEDIN), '_blank');
      }, 500);
      break;
    case 'github':
      writeLines(["Redirecting to github.com...", "<br>"]);
      setTimeout(() => {
        window.open('https://github.com/' + encodeURIComponent(GITHUB), '_blank');
      }, 500);
      break;
    case 'email':
      writeLines(["Redirecting to default email service...", "<br>"]);
      setTimeout(() => {
